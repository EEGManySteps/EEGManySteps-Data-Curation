...
bidsFolder      = fullfile(rootDir, 'BIDS-data-baseline');
...
for ID = IDs
    for session = sessions
        ...
        baseMarkers = find(strcmp('Baseline',{eeg.event.type})); 
        assert(numel(baseMarkers) == 2);
        tStart = eeg.event(baseMarkers(1)).latency/eeg.srate;
        tEnd = eeg.event(baseMarkers(2)).latency/eeg.srate;
        duration = tEnd - tStart;
        disp(['Baseline approx. ' num2str(round(duration/60)) ' minutes'])
        
        % compute difference offset 
        eegTime = datetime(eeg.etc.dateTime, ...
            'InputFormat','yyyy-MM-dd''T''HH:mm:ss.SSSSSS');
        
        imuTime = datetime(imu.IMU.dateTime, ...
            'InputFormat','yyyy-MM-dd''T''HH:mm:ss.SSSSSS');
        dt_seconds = seconds(eegTime - imuTime);
        dt_samples = dt_seconds*imu.IMU.samplingRate; % number of samples IMU is shifted by
        
        % crop out only the baseline
        cutEEG = eeg; 
        cutEEG.data = eeg.data(:, eeg.event(baseMarkers(1)).latency: eeg.event(baseMarkers(2)).latency); 
        cutEEG.times = eeg.times(:, eeg.event(baseMarkers(1)).latency: eeg.event(baseMarkers(2)).latency);
        cutEEG.pnts = numel(cutEEG.times); 
        cutIMU = imu; 
        cutIMU.IMU.dataArray = imu.IMU.dataArray(:,round(tStart*imu.IMU.samplingRate)+ round(dt_samples):round(tEnd*imu.IMU.samplingRate) + round(dt_samples),:);
        ...
        %--------------------------------------------------------------------------
        EEGftData           = eeglab2fieldtrip(cutEEG, 'raw', 'none');
        MotionftData        = struct_to_ft(cutIMU.IMU);
        ...
        % cfg.scans.acq_time                  = eeg.etc.dateTime; % synch already achieved above
        ...
        % time synch information in scans.tsv file
        %cfg.scans.acq_time  = imu.IMU.dateTime ; % synch already achieved above
